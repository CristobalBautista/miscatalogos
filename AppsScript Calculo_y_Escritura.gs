function calculateAndSelectFinalValues(entries, nombreMio, actualNombreCheck, actualNombreFinal) {
  ...
  const scoresWithEps = completedOnes.filter(e => e.score)
    .map(e => ({ score: e.score, episodes: (e.mediaType === 'movie' ? 3 : (e.episodes || '0')) }));
  const puntajeProm = completedOnes.length ? scoresWithEps.reduce((sum, e) => sum + (e.score * e.episodes), 0) / totalEps : primera.score;
...
function putValuesInSheet(sheet, fila, col, finalValues) {
  ...
  sheet.getRange(fila, col.MAL_Puntaje + 1).setValue(finalValues.puntajeProm.toFixed(2));
